...
<head>
    ...
    <title>
        {{ $config->get('ui.title') ?? config('app.name') . ' - API Docs' }}
    </title>
    ...
    <link
        rel="stylesheet"
        href="https://unpkg.com/@stoplight/elements@8.4.2/styles.min.css"
    >
    ...
    <script>
        ...
        window.fetch = async (url, options = {}) => {
            ...
            */
            ...
            const apiToken = localStorage.getItem('scramble_api_token');
            ...
            */
            ...
            try {
                ...
                */
                ...
                if (
                    url.toString().includes('/api/login') ||
                    url.toString().includes('/api/register')
                ) {
                    ...
                    if (data.token) {
                        ...
                        console.log(
                            'Scramble API token saved automatically'
                        );
                    }
                }
                ...
                /*
                |--------------------------------------------------------------------------
                | Auto Remove Token Logout
                |--------------------------------------------------------------------------
                */

                if (url.toString().includes('/api/logout')) {
...
<body style="height: 100vh; overflow-y: hidden">
    ...
    <elements-api
        ...
        @if($config->get('ui.hide_schemas'))
            hideSchemas="true"
        @endif

        @if($config->get('ui.logo'))
            logo="{{ $config->get('ui.logo') }}"
        @endif

        @if($config->get('ui.layout'))
